...
#ifndef PIFO_COMMON_SCHEDULE_HOSTSCHEDULE_H_
#define PIFO_COMMON_SCHEDULE_HOSTSCHEDULE_H_
 ...
 */
template<typename T>
class PIFOHostSchedule {
protected:
    /**
     * Schedule entries, that consist of a length in abstract time units and
     * a scheduled object.
     */
    std::vector<std::tuple<simtime_t, int, int, T>> entries;
...
public:
    ...
    virtual T getScheduledObject(unsigned int index) const {
        return std::get < 3 > (entries[index]);
    }
    ...
    virtual unsigned int getNumPacketsPerBurst(unsigned int index) const {
     ...
     *                        scheduled entry.
     */
    virtual void addEntry(simtime_t time, int size, int numPacketsPerBurst, T scheduledObject) {
        entries.push_back(std::make_tuple(time, size, numPacketsPerBurst, scheduledObject));
    }
};
