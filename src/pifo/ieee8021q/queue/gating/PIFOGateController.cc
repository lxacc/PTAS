...
#include "PIFOGateController.h"

namespace nesting {

Define_Module(PIFOGateController);


PIFOGateController::~PIFOGateController() {
...
void PIFOGateController::initialize(int stage) {
    ...
    //initialize schedule in second stage when clock is initialized
        ...
        currentSchedule = new PIFOSchedule<GateBitvector>();
...
int PIFOGateController::numInitStages() const {
    return INITSTAGE_LINK_LAYER + 1;
}

void PIFOGateController::handleMessage(cMessage *msg) {
...
void PIFOGateController::tick(IClock *clock, short kind) {
    Enter_Method("tick()");
    scheduleAt(simTime(), &updateScheduleMsg);
}

simtime_t PIFOGateController::scheduleNextTickEvent() {
...
unsigned int PIFOGateController::calculateMaxBit(int gateIndex) {
    double transmitRate;
...
void PIFOGateController::loadScheduleOrDefault(cXMLElement* xml) {
    PIFOSchedule<GateBitvector> *schedule = PIFOScheduleFactory::createGateBitvectorSchedule(xml);
...
void PIFOGateController::setGateStates(GateBitvector bitvector, bool release) {
    for (TransmissionGate* transmissionGate : transmissionGates) { 
        transmissionGate->setGateState(bitvector.test(transmissionGate->getIndex()), release);
    }
}

void PIFOGateController::updateSchedule()
{
