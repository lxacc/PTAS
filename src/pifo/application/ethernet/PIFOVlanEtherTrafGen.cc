...
#include "pifo/application/ethernet/PIFOVlanEtherTrafGen.h"
#include "pifo/application/ethernet/PIFOVlanEtherTrafGenSched.h"
#include "../../common/schedule/PIFOHostScheduleBuilder.h"
...
Define_Module(PIFOVlanEtherTrafGen);

void PIFOVlanEtherTrafGen::initialize(int stage) {
    ...
    if (stage == INITSTAGE_LOCAL) {
        ...
        registerService(*PIFOVlanEtherTrafGenSched::L2_PROTOCOL, nullptr, gate("in"));
        registerProtocol(*PIFOVlanEtherTrafGenSched::L2_PROTOCOL, gate("out"), nullptr);
    }
}

void PIFOVlanEtherTrafGen::sendBurstPackets() {
...
//        EV_INFO << ": !!!!!!!!!!!!!!!!!!!!!!CreationTime '" << timeTag->getCreationTime() << endl;
        datapacket->insertAtBack(payload);
        datapacket->removeTagIfPresent<PacketProtocolTag>();
        datapacket->addTagIfAbsent<PacketProtocolTag>()->setProtocol(PIFOVlanEtherTrafGenSched::L2_PROTOCOL);
