...
#ifndef PIFO_COMMON_SCHEDULE_PIFOSCHEDULE_H_
#define PIFO_COMMON_SCHEDULE_PIFOSCHEDULE_H_
...
class PIFOSchedule {
    /** Tuple consisting of a time interval and a scheduled object./ some Custom field / */
    struct ControlListEntry {
        simtime_t timeInterval;
        T scheduledObject;
        std::string macaddress;     //default : null
        short vlanId;                  //default : 0
    };
protected:
    /**
...
public:
    ...
    virtual std::string getMacAddress(unsigned index) const {
            return controlList[index].macaddress;
        }

    virtual short getVlanId(unsigned index) const {
                return controlList[index].vlanId;
            }

    virtual ControlListEntry getControlList(unsigned index) {
    ...
    virtual void addControlListEntry(simtime_t timeInterval, T scheduledObject,std::string macaddress= Default_Mac,short vlanId = Default_VlanId) {
...
template<typename T>
std::ostream& operator<<(std::ostream& stream, const PIFOSchedule<T>& schedule)
{
    return stream << "PIFOSchedule[baseTime=" << schedule.getBaseTime()
...
template<typename T>
std::ostream& operator<<(std::ostream& os, const std::shared_ptr<const PIFOSchedule<T>>& schedule)
{
    return os << *schedule;
}

template<typename T>
std::ostream& operator<<(std::ostream& os, const std::shared_ptr<PIFOSchedule<T>>& schedule)
{
    return os << *schedule;
}

template<typename T>
std::ostream& operator<<(std::ostream& os, const std::unique_ptr<const PIFOSchedule<T>>& schedule)
{
    return os << *schedule;
}

template<typename T>
std::ostream& operator<<(std::ostream& os, const std::unique_ptr<PIFOSchedule<T>>& schedule)
{
    return os << *schedule;
}
