...
#include "pifo/application/ethernet/PIFOVlanEtherTrafGenSched.h"
#include "pifo/common/schedule/PIFOHostScheduleBuilder.h"
...
Define_Module(PIFOVlanEtherTrafGenSched);

const Protocol* PIFOVlanEtherTrafGenSched::L2_PROTOCOL = &Protocol::nextHopForwarding;

PIFOVlanEtherTrafGenSched::~PIFOVlanEtherTrafGenSched() {
...
void PIFOVlanEtherTrafGenSched::initialize(int stage) {
    if (stage == INITSTAGE_LOCAL) {
        ...
        currentSchedule = std::unique_ptr < PIFOHostSchedule
                < Ieee8021QCtrl >> (new PIFOHostSchedule<Ieee8021QCtrl>());
...
int PIFOVlanEtherTrafGenSched::numInitStages() const {
    return INITSTAGE_LINK_LAYER + 1;
}

void PIFOVlanEtherTrafGenSched::handleMessage(cMessage *msg) {
...
void PIFOVlanEtherTrafGenSched::sendPacket(uint64_t scheduleIndexTx) {
    ...
    if (flowIdSeqNums.find(header.flowId) == flowIdSeqNums.end()) {
        flowIdSeqNums[header.flowId] = 1;
    }
    int burst = currentSchedule->getNumPacketsPerBurst(scheduleIndexTx % currentSchedule->size());
    for (int i = 0; i < burst; i++) {
        ...
        timeTag->setCreationTime(SIMTIME_DBL(simTime())+ (double)(len + 30 + 12)*8*i/1000000000); //30 bytes is 1 and 2 layer headers, 12 is IFG
        EV_INFO << ": !!!!!!!!!!!!!!!!!!!!!!CreationTime '" << simTime() << endl;
...
void PIFOVlanEtherTrafGenSched::receivePacket(Packet *pkt) {
...
void PIFOVlanEtherTrafGenSched::tick(IClock *clock, short kind) {
    Enter_Method("tick()");
...
void PIFOVlanEtherTrafGenSched::sendDelayed(cMessage *msg) {
...
simtime_t PIFOVlanEtherTrafGenSched::scheduleNextTickEvent() {
...
void PIFOVlanEtherTrafGenSched::loadScheduleOrDefault(cXMLElement* xml) {
    std::string hostName = this->getModuleByPath(par("hostModule"))->getFullName();
    PIFOHostSchedule<Ieee8021QCtrl>* schedule;
    ...
    for (cXMLElement* hostxml : xml->getChildren()) {
        if (strcmp(hostxml->getTagName(), "defaultcycle") != 0
            ...
            schedule = PIFOHostScheduleBuilder::createHostScheduleFromXML(hostxml,
                    xml);
    ...
    if (!realScheduleFound) {
        ...
        schedule = PIFOHostScheduleBuilder::createHostScheduleFromXML(defaultXml,
                xml);
    }
    std::unique_ptr<PIFOHostSchedule<Ieee8021QCtrl>> schedulePtr(schedule);
